	...
	public void transform(String sourceFile, String destFolder) {
		...
		for (WSDLOperation wsdlOperation : ops) {
			if(wsdlOperation == null){
				...
				File file = new File(destFolder + File.separator + name + ".owl");
				...
				for (WSDLParameter wsdlParameter : inputs) {
					...
					t.addInput(wsdlParameter, paramName, paramTypeURI, xsltTransformation);
				}
				...
				for (WSDLParameter wsdlParameter : outputs) {
					...
					t.addOutput(wsdlParameter, paramName, paramTypeURI, xsltTransformation);
				}
